  ...
  return (
    <div className="group relative h-full w-full border border-black rounded-lg ">
      <div className="absolute inset-0 flex flex-col justify-center items-center group-hover:invisible">
        ...
        <div className="h-1/4 w-full p-2 flex flex-row  m-0 flex-shrink space-x-1 items-center">
          {languages.map((language: string, index: number) => (
            <p key={index} className="p-1  bg bg-gray-200 rounded-lg font-mono sm:text-sm md:text-sm lg:text-sm">
              {language}
            </p>
          ))}
        </div>
